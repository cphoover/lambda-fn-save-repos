...
export default class RepoSaver {
	...
	_validate(repo) {
		this._validateRequired(repo, 'id', _.isInteger);
		...
		return Bluebird.try(() => {
			...
			if (this.signRequest) {
				...
				if (this.useEnvCreds) {
					...
					clientConfig.amazonES.credentials = this.myCredentials;
				} else {
					clientConfig.amazonES.accessKey = this.accessKey;
					clientConfig.amazonES.secretKey = this.secretKey;
				}
			}
