...
export default class RepoSaver {
	...
	_validateOptional(_repo, field, validator) {
		assert(!_repo[field] || validator(_repo[field]), `optional field ${_repo[field]} must be valid`);
	}

	_validateRequired(_repo, field, validator) {
		assert(validator(_repo[field]), `required field ${_repo[field]} must be valid`);
	}
	...
	run(event) {
		...
		if (this.signRequest) {
			clientConfig.connectionClass = httpAwsEs;
		...
		const repos = _.isArray(event.repos) ? event.repos : [event.repos];
